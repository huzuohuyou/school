Ext.define('manage.controller.Wx_new', {
	...
	refs : [ {
		...
		ref : 'newreadingGrid',
		selector : 'newreadinggrid'
	},{
		ref : 'newreadingViewForm',
		selector : 'newreadingviewform'
	}],
	views : ['wx_new.Form','wx_new.ViewForm','wx_new.ReadingViewForm','wx_new.NewReadingViewForm','wx_new.NewReadingForm'],
	init : function(app) {
		this.control({
			'wx_newgrid button[action=insert]' : {
				click : this.add
			},
			'newreadinggrid button[action=addReading]' : {
				click : this.addReading
			},
			'wx_newgrid button[action=query]' : {
				click : this.query
			},
			'wx_newgrid button[action=delete]' : {
				click : this.deleteWx_news
			},
			'newreadinggrid button[action=deleteReading]' : {
				click : this.deleteItems
			},
			'wx_newgrid actioncolumn[action=edit]' : {
				click : this.edit
			},
			'newreadinggrid actioncolumn[action=edit]' : {
				click : this.editReading
			},
			'wx_newgrid actioncolumn[action=yes]' : {
				click : this.yes
			},
			'newreadinggrid actioncolumn[action=showOrNot]' : {
				click : this.showOrNot
			},
			'newreadinggrid actioncolumn[action=view]' : {
				click : this.viewReadingDetail
			},
			'wx_newgrid actioncolumn[action=view]' : {
				click : this.viewDetail
			},
			'wx_newgrid actioncolumn[action=reading]' : {
				click : this.reading
			},
			'wx_newform button[action=submit]' : {
				click : this.submit
			},
			'wx_newreadingform button[action=submit]' : {
	...
	viewDetail : function(grid, rowIndex, colIndex, actionItem, event, record,
		...
		if (session.authority.indexOf('b350101') > -1) {
			var formWin = createWin('查看图文信息', 'wx_newviewform');
	...
	viewReadingDetail : function(grid, rowIndex, colIndex, actionItem, event, record,
			row) {
		if (session.authority.indexOf('b350101') > -1) {
			var formWin = createWin('查看图文信息', 'wx_newreadingviewform');
			formWin.down('form').loadRecord(record);
	...
	add : function(button) {
		if (session.authority.indexOf('b350102') > -1)
			createWin('添加图文', 'wx_newform');
		else {
			Ext.Msg.alert('系统提示', '您无权进行此项操作');
		}
	},
	addReading : function(button) {
		if (session.authority.indexOf('b350102') > -1)	{		
		    var formWin = createWin('添加相关阅读', 'wx_newreadingform');
		    var p_id = this.getNewreadingViewForm().form.findField('id').getValue();
	...
	edit : function(grid, rowIndex, colIndex, actionItem, event, record, row) {
		if (session.authority.indexOf('b350103') > -1) {
			var formWin = createWin('修改最新栏目消息', 'wx_newform');
			formWin.down('form').loadRecord(record);
	...
	editReading : function(grid, rowIndex, colIndex, actionItem, event, record, row) {
		if (session.authority.indexOf('b350103') > -1) {
			var formWin = createWin('修改相关阅读', 'wx_newreadingform');
			formWin.down('form').loadRecord(record);
	...
	deleteItems : function(button) {
	 ...
	 //网站管理员删除栏目下面的文章。
	deleteWx_news : function(button) {
    	if (session.authority.indexOf('b350104') > -1) {	
    		var grid = this.getWx_newGrid();
	...
	submitReading : function(button) {
		...
		var win = button.up('window');
		var grid = this.getNewreadingGrid();
	 ...
	 reading:function(grid, rowIndex, colIndex, actionItem, event, record, row) {
	        if (session.authority.indexOf('b350101') > -1) {
	            var formWin = createWin('相关阅读管理', 'newreadingviewform');
