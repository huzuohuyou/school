Ext.define('manage.controller.ExecuteActivity', {
    extend: 'Ext.app.Controller',
    refs: [{
        ref: 'executeactivityGrid',
        selector: 'executeactivitygrid'
    },{
        ref: 'stepGrid',
        selector: 'stepgrid'
    }],
    views: ['executeactivity.Form','executeactivity.ViewForm','executeactivity.WorkOrderForm',
    	'executeactivity.StepForm'],
    init: function(app) {
        this.control({
            'executeactivitygrid button[action=insert]': {
                click: this.add
            },
			'executeactivitygrid button[action=query]' : {
				click : this.query
			},
            'executeactivitygrid actioncolumn[action=edit]': {
                click: this.edit
            },
            'executeactivitygrid button[action=delete]': {
                click: this.deleteItems
            },
            'executeactivitygrid button[action=submit]': {
                click: this.submit
            },
            'executeactivitygrid actioncolumn[action=viewContract]': {
                click: this.viewContract
            },
            'executeactivitygrid actioncolumn[action=start]': {
                click: this.start
            },
            'executeactivitygrid actioncolumn[action=step]': {
                click: this.step
            },
            'executeactivitygrid actioncolumn[action=end]': {
                click: this.end
            },
            'executeactivitygrid actioncolumn[action=confirm]': {
                click: this.confirm
            },
            'executeactivitygrid actioncolumn[action=view]': {
                click: this.viewDetail
            },
            'executeactivitygrid actioncolumn[action=workOrder]': {
                click: this.workOrder
            },
            'executeactivitygrid actioncolumn[action=workOrderPass]': {
                click: this.workOrderPass
            },
            'stepgrid actioncolumn[action=activityStepExecute]': {
                click: this.activityStepExecute
            },
            'executeactivityworkorderform button[action=submit]': {
    ...
    viewDetail : function(grid, rowIndex, colIndex, actionItem, event, record,row) {
		...
		var formWin = createWin('查看活动', 'executeactivityviewform');
		formWin.down('form').loadRecord(record);
		
	},
    add: function(button) {
        if (session.authority.indexOf('b300102') > -1) createWin('添加活动', 'executeactivityform');
        else {
    ...
    workOrder: function(grid, rowIndex, colIndex, actionItem, event, record,
		...
		var formWin = createWin('活动工单', 'executeactivityworkorderform');
    ...
    wordordersubmit: function(button) {
        ...
        var win = button.up('window');
        var grid = this.getExecuteactivityGrid();
        var freshfn = function() {
            win.close();
            grid.getStore().reload();
        };
