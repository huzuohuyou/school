Ext.define('manage.controller.Wx_art', {
	...
	refs : [ {
		...
		ref : 'artreadingGrid',
		selector : 'artreadinggrid'
	},{
		ref : 'artreadingViewForm',
		selector : 'artreadingviewform'
	}],
	views : ['wx_art.Form','wx_art.ViewForm','wx_art.ArtReadingViewForm','wx_art.ArtReadingForm','wx_art.ArtViewForm'],
	init : function(app) {
		this.control({
			'wx_artgrid button[action=insert]' : {
				click : this.add
			},
			'artreadinggrid button[action=addReading]' : {
				click : this.addReading
			},
			'wx_artgrid button[action=query]' : {
				click : this.query
			},
			'wx_artgrid button[action=delete]' : {
				click : this.deleteWx_researchs
			},
			'artreadinggrid button[action=deleteReading]' : {
				click : this.deleteItems
			},
			'wx_artgrid actioncolumn[action=edit]' : {
				click : this.edit
			},
			'artreadinggrid actioncolumn[action=edit]' : {
				click : this.editReading
			},
			'wx_artgrid actioncolumn[action=yes]' : {
				click : this.yes
			},
			'artreadinggrid actioncolumn[action=showOrNot]' : {
				click : this.showOrNot
			},
			'artreadinggrid actioncolumn[action=view]' : {
				click : this.viewReadingDetail
			},
			'wx_artgrid actioncolumn[action=view]' : {
				click : this.viewDetail
			},
			'wx_artgrid actioncolumn[action=reading]' : {
				click : this.reading
			},
			'wx_artform button[action=submit]' : {
				click : this.submit
			},
			'artreadingform button[action=submit]' : {
	...
	viewDetail : function(grid, rowIndex, colIndex, actionItem, event, record,
		...
		if (session.authority.indexOf('b350901') > -1) {
			var formWin = createWin('查看图文信息', 'wx_artviewform');
	...
	viewReadingDetail : function(grid, rowIndex, colIndex, actionItem, event, record,
			row) {
		if (session.authority.indexOf('b350901') > -1) {
			var formWin = createWin('查看图文信息', 'wx_artreadingviewform');
			formWin.down('form').loadRecord(record);
	...
	add : function(button) {
		if (session.authority.indexOf('b350902') > -1)
			createWin('添加图文', 'wx_artform');
		else {
			Ext.Msg.alert('系统提示', '您无权进行此项操作');
		}
	},
	addReading : function(button) {
		if (session.authority.indexOf('b350902') > -1)	{		
		    var formWin = createWin('添加相关阅读', 'artreadingform');
		    var p_id = this.getArtreadingViewForm().form.findField('id').getValue();
	...
	edit : function(grid, rowIndex, colIndex, actionItem, event, record, row) {
		if (session.authority.indexOf('b350903') > -1) {
			var formWin = createWin('修改活动消息', 'wx_artform');
			formWin.down('form').loadRecord(record);
	...
	editReading : function(grid, rowIndex, colIndex, actionItem, event, record, row) {
		if (session.authority.indexOf('b350903') > -1) {
			var formWin = createWin('修改相关阅读', 'artreadingform');
			formWin.down('form').loadRecord(record);
	...
	deleteWx_researchs : function(button) {
    	if (session.authority.indexOf('b350904') > -1) {	
    		var grid = this.getWx_artGrid();
	...
	submitReading : function(button) {
		...
		var win = button.up('window');
		var grid = this.getArtreadingGrid();	
		var freshfn = function() {
			win.close();
			grid.getStore().reload();
		};
	 ...
	 reading:function(grid, rowIndex, colIndex, actionItem, event, record, row) {
	        if (session.authority.indexOf('b350901') > -1) {
	            var formWin = createWin('相关阅读管理', 'artreadingviewform');
