Ext.define('manage.view.newactivity_execute.WorkOrderForm', {
	extend : 'Ext.form.FormPanel',
	alias : 'widget.executenewworkorderform',
	requires : [ 'manage.view.ux.ComboBox','manage.view.ux.UEditor' ],
//	bodyPadding : 10,
	header : false,
	id : 'executenewworkorderform',
	buttonAlign : 'center',
										    ...
										    items:[                                           {
									            	...
									            	
									           {
									            ...
									            items:[{
						 							...
						 							if(status == 5)
						 							{	
							 						fileUpload(button,'cdbg_f_id','cdbg_f_name','cdbg_f_nameLink');
						 							}
						 							else{
						 								
						 							Ext.Msg.alert('系统提示', '当前阶段不允许上传踩点报告');	
							 						...
							 						if(status == 5)
							 						{	
							 						fileUpload(button,'yjya_f_id','yjya_f_name','yjya_f_nameLink');
							 						}
							 						else{
							 							
							 						Ext.Msg.alert('系统提示','当前阶段不允许上传应急预案');	
							 						...
							 						fileUpload(button,'hdxz_f_id','hdxz_f_name','hdxz_f_nameLink');
								 					}
								 					else{
								 					
								 					Ext.Msg.alert('系统提示','当前阶段不允许上传活动须知');		
									 				...
									 				if(status == 5)
									 				{	
							 						fileUpload(button,'zxbz_f_id','zxbz_f_name','zxbz_f_nameLink');
									 				}
									 				else{
									 				
									 				Ext.Msg.alert('系统提示','当前阶段不允许上传执行步骤')
										 			...
										 			if(status == 5)
										 			{	
							 						fileUpload(button,'fkjg_f_id','fkjg_f_name','fkjg_f_nameLink');
										 			}
										 			
										 			else{
										 			
										 			Ext.Msg.alert('系统提示','当前阶段不允许上传反馈结果')	
									            ...
									            items:[{
													...
													handler : function(button){
														if(status==7){
														button.up('form').form.findField('cdbg_f_id').setValue('');
														button.up('form').form.findField('cdbg_f_name').setValue('');
														button.up('form').form.findField('cdbg_f_nameLink').setValue('');
														}
													...
													handler : function(button){
														...
														if(status==7){
														button.up('form').form.findField('yjya_f_id').setValue('');
														button.up('form').form.findField('yjya_f_name').setValue('');
														button.up('form').form.findField('yjya_f_nameLink').setValue('');
														}
													...
													handler : function(button){
														if(status==7){
														button.up('form').form.findField('hdxz_f_id').setValue('');
														button.up('form').form.findField('hdxz_f_name').setValue('');
														button.up('form').form.findField('hdxz_f_nameLink').setValue('');
														}
														else{
														Ext.Msg.alert('系统提示','当前工单状态不能够进行文件删除');																
														}
													...
													handler : function(button){
														if(status==7){
														button.up('form').form.findField('zxbz_f_id').setValue('');
														button.up('form').form.findField('zxbz_f_name').setValue('');
														button.up('form').form.findField('zxbz_f_nameLink').setValue('');
														}
														else{
														Ext.Msg.alert('系统提示','当前工单状态不能够进行文件删除');	
														}
													...
													handler : function(button){
														if(status==7){
														button.up('form').form.findField('fkjg_f_id').setValue('');
														button.up('form').form.findField('fkjg_f_name').setValue('');
														button.up('form').form.findField('fkjg_f_nameLink').setValue('');
														}
														else{
														Ext.Msg.alert('系统提示','当前工单状态不能够进行文件删除');		
														}
	...
	upLoadForm.submit({
		...
		success : function(form, action) {
			top.Ext.Msg.alert('上传成功', action.result.msg == null ? '上传成功'
				...
				var replyForm = Ext.getCmp('executenewworkorderform').form;
