Ext.define('manage.controller.Wx_research_in', {
	extend : 'Ext.app.Controller',
	refs : [ {
		ref : 'wx_research_inGrid',
		selector : 'wx_research_ingrid'
	},{
		ref : 'researchinreadingGrid',
		selector : 'researchinreadinggrid'
	},{
		ref : 'researchinreadingViewForm',
		selector : 'researchinreadingviewform'
	}],
	views : ['wx_research_in.Form','wx_research_in.ViewForm','wx_research_in.ResearchInReadingViewForm','wx_research_in.ResearchInReadingForm','wx_research_in.ResearchInViewForm'],
	init : function(app) {
		this.control({
			'wx_research_ingrid button[action=insert]' : {
				click : this.add
			},
			'researchinreadinggrid button[action=addReading]' : {
				click : this.addReading
			},
			'wx_research_ingrid button[action=query]' : {
				click : this.query
			},
			'wx_research_ingrid button[action=delete]' : {
				click : this.deleteWx_researchs
			},
			'researchinreadinggrid button[action=deleteReading]' : {
				click : this.deleteItems
			},
			'wx_research_ingrid actioncolumn[action=edit]' : {
				click : this.edit
			},
			'researchinreadinggrid actioncolumn[action=edit]' : {
				click : this.editReading
			},
			'wx_research_ingrid actioncolumn[action=yes]' : {
				click : this.yes
			},
			'researchinreadinggrid actioncolumn[action=showOrNot]' : {
				click : this.showOrNot
			},
			'researchinreadinggrid actioncolumn[action=view]' : {
				click : this.viewReadingDetail
			},
			'wx_research_ingrid actioncolumn[action=view]' : {
				click : this.viewDetail
			},
			'wx_research_ingrid actioncolumn[action=reading]' : {
				click : this.reading
			},
			'wx_research_inform button[action=submit]' : {
				click : this.submit
			},
			'researchinreadingform button[action=submit]' : {
	...
	yes : function(grid, rowIndex, colIndex, actionItem, event, record,
			row) {
		if (session.authority.indexOf('b350203') > -1) {
	...
	viewDetail : function(grid, rowIndex, colIndex, actionItem, event, record,
			row) {
		if (session.authority.indexOf('b350201') > -1) {
			var formWin = createWin('查看图文信息', 'wx_research_inviewform');
	...
	viewReadingDetail : function(grid, rowIndex, colIndex, actionItem, event, record,
			row) {
		if (session.authority.indexOf('b350201') > -1) {
			var formWin = createWin('查看图文信息', 'researchinviewform');
			formWin.down('form').loadRecord(record);
	...
	add : function(button) {
		if (session.authority.indexOf('b350202') > -1)
			createWin('添加图文', 'wx_research_inform');
		else {
			Ext.Msg.alert('系统提示', '您无权进行此项操作');
		}
	},
	addReading : function(button) {
		if (session.authority.indexOf('b350202') > -1)	{		
		    var formWin = createWin('添加相关阅读', 'researchinreadingform');
		    var p_id = this.getResearchinreadingViewForm().form.findField('id').getValue();
	...
	edit : function(grid, rowIndex, colIndex, actionItem, event, record, row) {
		if (session.authority.indexOf('b350103') > -1) {
			var formWin = createWin('修改国内研学消息', 'wx_research_inform');
			formWin.down('form').loadRecord(record);
	...
	editReading : function(grid, rowIndex, colIndex, actionItem, event, record, row) {
		if (session.authority.indexOf('b350203') > -1) {
			var formWin = createWin('修改相关阅读', 'researchinreadingform');
			formWin.down('form').loadRecord(record);
	...
	deleteItems : function(button) {
		if (session.authority.indexOf('b350204') > -1) {
			var grid = button.up('grid');
	...
	deleteWx_researchs : function(button) {
    	if (session.authority.indexOf('b350204') > -1) {	
    		var grid = this.getWx_research_inGrid();
	...
	submitReading : function(button) {
		...
		var win = button.up('window');
		var grid = this.getResearchinreadingGrid();	
		var freshfn = function() {
			win.close();
			grid.getStore().reload();
		};
	 ...
	 reading:function(grid, rowIndex, colIndex, actionItem, event, record, row) {
	        if (session.authority.indexOf('b350201') > -1) {
	            var formWin = createWin('相关阅读管理', 'researchinreadingviewform');
