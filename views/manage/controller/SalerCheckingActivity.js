Ext.define('manage.controller.SalerCheckingActivity', {
    extend: 'Ext.app.Controller',
    refs: [{
        ref: 'salercheckingactivityGrid',
        selector: 'salercheckingactivitygrid'
    },{
        ref: 'stepGrid',
        selector: 'stepgrid'
    }],
    views: ['salercheckingactivity.Form','salercheckingactivity.ViewForm','salercheckingactivity.WorkOrderForm','salercheckingactivity.StepForm'],
    init: function(app) {
        this.control({
            'salercheckingactivitygrid button[action=insert]': {
                click: this.add
            },
			'salercheckingactivitygrid button[action=query]' : {
				click : this.query
			},
            'salercheckingactivitygrid actioncolumn[action=edit]': {
                click: this.edit
            },
            'salercheckingactivitygrid button[action=delete]': {
                click: this.deleteItems
            },
            'activityform button[action=submit]': {
                click: this.submit
            },
            'salercheckingactivitygrid actioncolumn[action=uploadFile]': {
                click: this.uploadFile
            },
            'salercheckingactivitygrid actioncolumn[action=executeActivity]': {
                click: this.executeActivity
            },
            'salercheckingactivitygrid actioncolumn[action=view]': {
                click: this.viewDetail
            },
            'salercheckingactivitygrid actioncolumn[action=workOrder]': {
                click: this.workOrder
            },
            'salercheckingactivitygrid actioncolumn[action=check]': {
                click: this.check
            },
            'salercheckingactivitygrid actioncolumn[action=step]': {
                click: this.step
            },
            'salercheckingworkorderform button[action=submit]': {
    ...
    query: function(button) {
        if (session.authority.indexOf('b310301') > -1) {
    ...
    workOrder: function(grid, rowIndex, colIndex, actionItem, event, record,
    	...
    	if (session.authority.indexOf('b310101') > -1) {
			var formWin = createWin('活动工单', 'salercheckingworkorderform');
