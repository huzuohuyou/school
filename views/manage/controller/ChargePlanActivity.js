Ext.define('manage.controller.ChargePlanActivity', {
    extend: 'Ext.app.Controller',
    refs: [{
        ref: 'chargeplanactivityGrid',
        selector: 'chargeplanactivitygrid'
    },{
        ref: 'chargeplanactivitystepGrid',
        selector: 'chargeplanactivitystepgrid'
    }],
    views: ['chargeplanactivity.Form','chargeplanactivity.ViewForm','chargeplanactivity.WorkOrderForm','chargeplanactivity.StepForm'],
    init: function(app) {
        this.control({
            'chargeplanactivitygrid button[action=insert]': {
                click: this.add
            },
			'chargeplanactivitygrid button[action=query]' : {
				click : this.query
			},
            'chargeplanactivitygrid actioncolumn[action=edit]': {
                click: this.edit
            },
            'chargeplanactivitygrid button[action=delete]': {
                click: this.deleteItems
            },
            'chargeplanactivitygrid button[action=submit]': {
                click: this.submit
            },
            'chargeplanactivitygrid actioncolumn[action=viewContract]': {
                click: this.viewContract
            },
            'chargeplanactivitygrid actioncolumn[action=planePass]': {
                click: this.planePass
            },
            'chargeplanactivitygrid actioncolumn[action=view]': {
                click: this.viewDetail
            },
            'chargeplanactivitygrid actioncolumn[action=workOrder]': {
                click: this.workOrder
            },
            'chargeplanactivitygrid actioncolumn[action=workOrderPass]': {
                click: this.workOrderPass
            },
            'chargeplanactivitygrid actioncolumn[action=step]': {
                click: this.step
            },
            'chargeplanactivityworkorderform button[action=submit]': {
    ...
    step: function(grid, rowIndex, colIndex, actionItem, event, record, row) {
    	...
    	var formWin = createWin('活动步骤', 'chargeplanactivitystepform');
    ...
    viewDetail : function(grid, rowIndex, colIndex, actionItem, event, record,
		...
		if (session.authority.indexOf('b300105') > -1) {
			var formWin = createWin('查看备注', 'chargeplanactivityviewform');
    ...
    add: function(button) {
        if (session.authority.indexOf('b300102') > -1) createWin('添加活动', 'chargeplanactivityform');
        else {
            Ext.Msg.alert('系统提示', '您无权进行此项操作');
        }
    },
    ...
    workOrder: function(grid, rowIndex, colIndex, actionItem, event, record,
    	...
    	if (session.authority.indexOf('b300101') > -1) {
			var formWin = createWin('活动工单', 'chargeplanactivityworkorderform');
