Ext.define('manage.controller.Activity', {
    ...
    views: ['activity.Form','activity.ViewForm','activity.WorkOrderForm','activity.StepForm'],
    init: function(app) {
        this.control({
            'activitygrid button[action=insert]': {
                click: this.add
            },
			'activitygrid button[action=query]' : {
				click : this.query
			},
            'activitygrid actioncolumn[action=edit]': {
                click: this.edit
            },
            'activitygrid button[action=delete]': {
                click: this.deleteItems
            },
            'activityform button[action=submit]': {
                click: this.submit
            },
            'activitygrid actioncolumn[action=uploadFile]': {
                click: this.uploadFile
            },
            'activitygrid actioncolumn[action=executeActivity]': {
                click: this.executeActivity
            },
            'activitygrid actioncolumn[action=view]': {
                click: this.viewDetail
            },
            'activitygrid actioncolumn[action=workOrder]': {
                click: this.workOrder
            },
            'activitygrid actioncolumn[action=check]': {
                click: this.check
            },
            'activitygrid actioncolumn[action=step]': {
                click: this.step
            },
            'workorderform button[action=submit]': {
    ...
    add: function(button) {
        if (session.authority.indexOf('b310102') > -1) createWin('添加活动', 'testform');
        else {
            Ext.Msg.alert('系统提示', '您无权进行此项操作');
        }
    },
    ...
    workOrder: function(grid, rowIndex, colIndex, actionItem, event, record,
    	...
    	if (session.authority.indexOf('b310101') > -1) {
			var formWin = createWin('活动工单', 'workorderform');
			formWin.down('form').loadRecord(record);
