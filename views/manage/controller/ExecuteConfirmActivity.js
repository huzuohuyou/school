Ext.define('manage.controller.ExecuteConfirmActivity', {
    extend: 'Ext.app.Controller',
    refs: [{
        ref: 'executeconfirmactivityGrid',
        selector: 'executeconfirmactivitygrid'
    },{
        ref: 'executeconfirmactivitystepGrid',
        selector: 'executeconfirmactivitystepgrid'
    }],
    views: ['executeconfirmactivity.Form','executeconfirmactivity.ViewForm','executeconfirmactivity.WorkOrderForm',
    	'executeconfirmactivity.StepForm'],
    init: function(app) {
        this.control({
            'executeconfirmactivitygrid button[action=insert]': {
                click: this.add
            },
			'executeconfirmactivitygrid button[action=query]' : {
				click : this.query
			},
            'executeconfirmactivitygrid actioncolumn[action=edit]': {
                click: this.edit
            },
            'executeconfirmactivitygrid button[action=delete]': {
                click: this.deleteItems
            },
            'executeconfirmactivitygrid button[action=submit]': {
                click: this.submit
            },
            'executeconfirmactivitygrid actioncolumn[action=viewContract]': {
                click: this.viewContract
            },
            'executeconfirmactivitygrid actioncolumn[action=start]': {
                click: this.start
            },
            'executeconfirmactivitygrid actioncolumn[action=step]': {
                click: this.step
            },
            'executeconfirmactivitygrid actioncolumn[action=end]': {
                click: this.end
            },
            'executeconfirmactivitygrid actioncolumn[action=confirm]': {
                click: this.confirm
            },
            'executeconfirmactivitygrid actioncolumn[action=view]': {
                click: this.viewDetail
            },
            'executeconfirmactivitygrid actioncolumn[action=workOrder]': {
                click: this.workOrder
            },
            'executeconfirmactivitygrid actioncolumn[action=workOrderPass]': {
                click: this.workOrderPass
            },
            'executeconfirmactivitystepgrid actioncolumn[action=activityStepExecute]': {
                click: this.activityStepExecute
            },
            'executeconfirmactivityworkorderform button[action=submit]': {
    ...
    viewDetail : function(grid, rowIndex, colIndex, actionItem, event, record,row) {
		...
		var formWin = createWin('查看活动', 'executeconfirmactivityviewform');
		formWin.down('form').loadRecord(record);
		
	},
    add: function(button) {
        if (session.authority.indexOf('b300102') > -1) createWin('添加活动', 'executeconfirmactivityform');
        else {
    ...
    edit: function(grid, rowIndex, colIndex, actionItem, event, record, row) {
        if (session.authority.indexOf('b300103') > -1) {
            var formWin = createWin('修改活动', 'executeconfirmactivityform');
            formWin.down('form').loadRecord(record);
        } else {
            Ext.Msg.alert('系统提示', '您无权进行此项操作');
        }
    ...
    workOrder: function(grid, rowIndex, colIndex, actionItem, event, record,
		...
		var formWin = createWin('活动工单', 'executeconfirmactivityworkorderform');
    ...
    step: function(grid, rowIndex, colIndex, actionItem, event, record, row) {
    	var formWin = createWin('活动步骤', 'executeconfirmactivitystepform');
    ...
    submit: function(button) {
        ...
        var grid = this.getExecuteconfirmactivityGrid();
