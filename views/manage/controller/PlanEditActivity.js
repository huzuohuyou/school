Ext.define('manage.controller.PlanEditActivity', {
    extend: 'Ext.app.Controller',
    refs: [{
        ref: 'planeditactivityGrid',
        selector: 'planeditactivitygrid'
    },{
        ref: 'planeditactivitygstepGrid',
        selector: 'planeditactivitygstepgrid'
    }],
    views: ['planeditactivity.Form','planeditactivity.ViewForm','planeditactivity.WorkOrderForm','planeditactivity.StepForm'],
    init: function(app) {
        this.control({
            'planeditactivitygrid button[action=insert]': {
                click: this.add
            },
			'planeditactivitygrid button[action=query]' : {
				click : this.query
			},
            'planeditactivitygrid actioncolumn[action=edit]': {
                click: this.edit
            },
            'planeditactivitygrid button[action=delete]': {
                click: this.deleteItems
            },
            'planeditactivitygrid button[action=submit]': {
                click: this.submit
            },
            'planeditactivitygrid actioncolumn[action=viewContract]': {
                click: this.viewContract
            },
            'planeditactivitygrid actioncolumn[action=commitPlane]': {
                click: this.commitPlane
            },
            'planeditactivitygrid actioncolumn[action=view]': {
                click: this.viewDetail
            },
            'planeditactivitygrid actioncolumn[action=workOrder]': {
                click: this.workOrder
            },
            'planeditactivitygrid actioncolumn[action=workOrderPass]': {
                click: this.workOrderPass
            },
            'planeditactivitygrid actioncolumn[action=step]': {
                click: this.step
            },
            'planeditactivityworkorderform button[action=submit]': {
    ...
    step: function(grid, rowIndex, colIndex, actionItem, event, record, row) {
    	...
    	var formWin = createWin('活动步骤', 'planeditactivitystepform');
    ...
    viewDetail : function(grid, rowIndex, colIndex, actionItem, event, record,row) {
		...
		var formWin = createWin('查看活动', 'planeditactivityviewform');
		formWin.down('form').loadRecord(record);
		
	},
    add: function(button) {
        if (session.authority.indexOf('b300102') > -1) createWin('添加活动', 'planeditactivityform');
        else {
            Ext.Msg.alert('系统提示', '您无权进行此项操作');
        }
    },
    query: function(button) {
        if (session.authority.indexOf('b320201') > -1) {
    ...
    edit: function(grid, rowIndex, colIndex, actionItem, event, record, row) {
        if (session.authority.indexOf('b300103') > -1) {
            var formWin = createWin('修改活动', 'planeditactivityform');
            formWin.down('form').loadRecord(record);
        } else {
            Ext.Msg.alert('系统提示', '您无权进行此项操作');
        }
    ...
    workOrder: function(grid, rowIndex, colIndex, actionItem, event, record,
		...
		var formWin = createWin('活动工单', 'planeditactivityworkorderform');
    ...
    commitPlane : function(grid, rowIndex, colIndex, actionItem, event, record,row) {
       
    	
    		if (record.data.status == 2 ||record.data.status == '2') {
    			if(record.data.w_planIntroduction == null ||record.data.w_planIntroduction == ''){
    				Ext.Msg.alert('系统提示', '请先提交策划方案!');
    				return;
    			}
    ...
    submit: function(button) {
        ...
        var grid = this.getPlaneditActivityGrid();
