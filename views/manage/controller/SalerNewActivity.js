Ext.define('manage.controller.SalerNewActivity', {
    ...
    views: ['salernewactivity.Form','salernewactivity.ViewForm','salernewactivity.WorkOrderForm','salernewactivity.StepForm'],
    init: function(app) {
        this.control({
            ...
            'salernewactivityform button[action=submit]': {
                click: this.submit
            },
            'newactivitygrid actioncolumn[action=uploadFile]': {
                click: this.uploadFile
            },
            'newactivitygrid actioncolumn[action=executeActivity]': {
                click: this.executeActivity
            },
            'newactivitygrid actioncolumn[action=view]': {
                click: this.viewDetail
            },
            'newactivitygrid actioncolumn[action=workOrder]': {
                click: this.workOrder
            },
            'newactivitygrid actioncolumn[action=check]': {
                click: this.check
            },
            'newactivitygrid actioncolumn[action=step]': {
                click: this.step
            },
            'salernewworkorderform button[action=submit]': {
    ...
    add: function(button) {
        if (session.authority.indexOf('b310102') > -1) createWin('添加活动', 'salernewactivityform');
        else {
            Ext.Msg.alert('系统提示', '您无权进行此项操作');
        }
    },
    query: function(button) {
        if (session.authority.indexOf('b310201') > -1) {
    ...
    edit: function(grid, rowIndex, colIndex, actionItem, event, record, row) {
        if (session.authority.indexOf('b310103') > -1) {
            var formWin = createWin('修改活动', 'salernewactivityform');
            formWin.down('form').loadRecord(record);
        } else {
            Ext.Msg.alert('系统提示', '您无权进行此项操作');
        }
    ...
    workOrder: function(grid, rowIndex, colIndex, actionItem, event, record,
    	...
    	if (session.authority.indexOf('b310101') > -1) {
			var formWin = createWin('活动工单', 'salernewworkorderform');
    ...
    wordordersubmit: function(button) {
        ...
        var action;
        var funcId;
        var  w_schoolName = form.findField('w_schoolId').getDisplayValue();
