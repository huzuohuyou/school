 ...
 */
Ext.define('manage.view.ux.ComboBoxTree',{
	...
	createPicker:function(){
		...
		self.picker = Ext.create('Ext.tree.Panel',{
			...
			viewConfig: {
				...
				if (self.multiSelect) {
					...
					var records = self.picker.getView().getChecked(), names = [], values = [], paths = [];
			    	Ext.Array.each(records, function(rec) {
			    		names.push(rec.get('text'));
			    		values.push(rec.get('id'));
			    		if(paths.indexOf(rec.get('path'))<0)
			    			paths.push(rec.get('path'));
			    		
			    	});
					self.pathArray = paths;
		...
		self.picker.on({
			itemclick: function (view,recore,item,index,e,object) {
					if (!self.multiSelect) {
						if(self.onlyLeaf && !recore.get('leaf')) {
							
						}else{
							self.submitValue = recore.get('id');
							self.setValue(recore.get('text')); 
							self.setPathValue(recore.get('path'));
