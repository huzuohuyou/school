Ext.define('manage.controller.PlanActivity', {
    extend: 'Ext.app.Controller',
    refs: [{
        ref: 'planactivityGrid',
        selector: 'planactivitygrid'
    },{
        ref: 'stepGrid',
        selector: 'stepgrid'
    }],
    views: ['planactivity.Form','planactivity.ViewForm','planactivity.WorkOrderForm','planactivity.StepForm'],
    init: function(app) {
        this.control({
            'planactivitygrid button[action=insert]': {
                click: this.add
            },
			'planactivitygrid button[action=query]' : {
				click : this.query
			},
            'planactivitygrid actioncolumn[action=edit]': {
                click: this.edit
            },
            'planactivitygrid button[action=delete]': {
                click: this.deleteItems
            },
            'planactivitygrid button[action=submit]': {
                click: this.submit
            },
            'planactivitygrid actioncolumn[action=viewContract]': {
                click: this.viewContract
            },
            'planactivitygrid actioncolumn[action=commitPlane]': {
                click: this.commitPlane
            },
            'planactivitygrid actioncolumn[action=view]': {
                click: this.viewDetail
            },
            'planactivitygrid actioncolumn[action=workOrder]': {
                click: this.workOrder
            },
            'planactivitygrid actioncolumn[action=workOrderPass]': {
                click: this.workOrderPass
            },
            'planactivitygrid actioncolumn[action=step]': {
                click: this.step
            },
            'planactivityworkorderform button[action=submit]': {
    ...
    viewDetail : function(grid, rowIndex, colIndex, actionItem, event, record,row) {
		...
		var formWin = createWin('查看活动', 'planactivityviewform');
		formWin.down('form').loadRecord(record);
		
	},
    add: function(button) {
        if (session.authority.indexOf('b300102') > -1) createWin('添加活动', 'planactivityform');
        else {
            Ext.Msg.alert('系统提示', '您无权进行此项操作');
        }
    },
    query: function(button) {
        if (session.authority.indexOf('b320101') > -1) {
    ...
    edit: function(grid, rowIndex, colIndex, actionItem, event, record, row) {
        if (session.authority.indexOf('b300103') > -1) {
            var formWin = createWin('修改活动', 'planactivityform');
            formWin.down('form').loadRecord(record);
        } else {
            Ext.Msg.alert('系统提示', '您无权进行此项操作');
        }
    ...
    workOrder: function(grid, rowIndex, colIndex, actionItem, event, record,
		...
		var formWin = createWin('活动工单', 'planactivityworkorderform');
    ...
    submit: function(button) {
        ...
        var grid = this.getPlanActivityGrid();
