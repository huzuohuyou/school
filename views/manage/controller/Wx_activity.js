Ext.define('manage.controller.Wx_activity', {
	extend : 'Ext.app.Controller',
	refs : [ {
		ref : 'wx_activityGrid',
		selector : 'wx_activitygrid'
	},{
		ref : 'activityreadingGrid',
		selector : 'activityreadinggrid'
	},{
		ref : 'activityreadingViewForm',
		selector : 'activityreadingviewform'
	}],
	views : ['wx_activity.Form','wx_activity.ViewForm','wx_activity.ActivityReadingViewForm','wx_activity.ActivityReadingForm','wx_activity.ActivityViewForm'],
	init : function(app) {
		this.control({
			'wx_activitygrid button[action=insert]' : {
				click : this.add
			},
			'activityreadinggrid button[action=addReading]' : {
				click : this.addReading
			},
			'wx_activitygrid button[action=query]' : {
				click : this.query
			},
			'wx_activitygrid button[action=delete]' : {
				click : this.deleteWx_researchs
			},
			'activityreadinggrid button[action=deleteReading]' : {
				click : this.deleteItems
			},
			'wx_activitygrid actioncolumn[action=edit]' : {
				click : this.edit
			},
			'activityreadinggrid actioncolumn[action=edit]' : {
				click : this.editReading
			},
			'wx_activitygrid actioncolumn[action=yes]' : {
				click : this.yes
			},
			'activityreadinggrid actioncolumn[action=showOrNot]' : {
				click : this.showOrNot
			},
			'activityreadinggrid actioncolumn[action=view]' : {
				click : this.viewReadingDetail
			},
			'wx_activitygrid actioncolumn[action=view]' : {
				click : this.viewDetail
			},
			'wx_activitygrid actioncolumn[action=reading]' : {
				click : this.reading
			},
			'wx_activityform button[action=submit]' : {
				click : this.submit
			},
			'activityreadingform button[action=submit]' : {
	...
	yes : function(grid, rowIndex, colIndex, actionItem, event, record,
			row) {
		if (session.authority.indexOf('b350503') > -1) {
	...
	viewDetail : function(grid, rowIndex, colIndex, actionItem, event, record,
			row) {
		if (session.authority.indexOf('b350501') > -1) {
			var formWin = createWin('查看图文信息', 'wx_activityviewform');
	...
	viewReadingDetail : function(grid, rowIndex, colIndex, actionItem, event, record,
			row) {
		if (session.authority.indexOf('b350501') > -1) {
			var formWin = createWin('查看图文信息', 'wxactivityreadingviewform');
			formWin.down('form').loadRecord(record);
	...
	add : function(button) {
		if (session.authority.indexOf('b350502') > -1)
			createWin('添加图文', 'wx_activityform');
		else {
			Ext.Msg.alert('系统提示', '您无权进行此项操作');
		}
	},
	addReading : function(button) {
		if (session.authority.indexOf('b350502') > -1)	{		
		    var formWin = createWin('添加相关阅读', 'activityreadingform');
		    var p_id = this.getActivityreadingViewForm().form.findField('id').getValue();
	...
	edit : function(grid, rowIndex, colIndex, actionItem, event, record, row) {
		if (session.authority.indexOf('b350503') > -1) {
			var formWin = createWin('修改活动消息', 'wx_activityform');
			formWin.down('form').loadRecord(record);
	...
	editReading : function(grid, rowIndex, colIndex, actionItem, event, record, row) {
		if (session.authority.indexOf('b350503') > -1) {
			var formWin = createWin('修改相关阅读', 'activityreadingform');
			formWin.down('form').loadRecord(record);
	...
	deleteWx_researchs : function(button) {
    	if (session.authority.indexOf('b350504') > -1) {	
    		var grid = this.getWx_activityGrid();
	...
	submitReading : function(button) {
		...
		var win = button.up('window');
		var grid = this.getActivityreadingGrid();	
		var freshfn = function() {
			win.close();
			grid.getStore().reload();
		};
	 ...
	 reading:function(grid, rowIndex, colIndex, actionItem, event, record, row) {
	        if (session.authority.indexOf('b350501') > -1) {
	            var formWin = createWin('相关阅读管理', 'activityreadingviewform');
