Ext.define('manage.controller.Wx_sport', {
	extend : 'Ext.app.Controller',
	refs : [ {
		ref : 'wx_sportGrid',
		selector : 'wx_sportgrid'
	},{
		ref : 'sportreadingGrid',
		selector : 'sportreadinggrid'
	},{
		ref : 'sportreadingViewForm',
		selector : 'sportreadingviewform'
	}],
	views : ['wx_sport.Form','wx_sport.ViewForm','wx_sport.SportReadingViewForm','wx_sport.SportReadingForm','wx_sport.SportViewForm'],
	init : function(app) {
		this.control({
			'wx_sportgrid button[action=insert]' : {
				click : this.add
			},
			'sportreadinggrid button[action=addReading]' : {
				click : this.addReading
			},
			'wx_sportgrid button[action=query]' : {
				click : this.query
			},
			'wx_sportgrid button[action=delete]' : {
				click : this.deleteWx_researchs
			},
			'sportreadinggrid button[action=deleteReading]' : {
				click : this.deleteItems
			},
			'wx_sportgrid actioncolumn[action=edit]' : {
				click : this.edit
			},
			'sportreadinggrid actioncolumn[action=edit]' : {
				click : this.editReading
			},
			'wx_sportgrid actioncolumn[action=yes]' : {
				click : this.yes
			},
			'sportreadinggrid actioncolumn[action=showOrNot]' : {
				click : this.showOrNot
			},
			'sportreadinggrid actioncolumn[action=view]' : {
				click : this.viewReadingDetail
			},
			'wx_sportgrid actioncolumn[action=view]' : {
				click : this.viewDetail
			},
			'wx_sportgrid actioncolumn[action=reading]' : {
				click : this.reading
			},
			'wx_sportform button[action=submit]' : {
				click : this.submit
			},
			'sportreadingform button[action=submit]' : {
	...
	viewDetail : function(grid, rowIndex, colIndex, actionItem, event, record,
		...
		if (session.authority.indexOf('b351001') > -1) {
			var formWin = createWin('查看图文信息', 'wx_sportviewform');
	...
	viewReadingDetail : function(grid, rowIndex, colIndex, actionItem, event, record,
			row) {
		if (session.authority.indexOf('b351001') > -1) {
			var formWin = createWin('查看图文信息', 'wx_sportreadingviewform');
			formWin.down('form').loadRecord(record);
	...
	add : function(button) {
		if (session.authority.indexOf('b351002') > -1)
			createWin('添加图文', 'wx_sportform');
		else {
			Ext.Msg.alert('系统提示', '您无权进行此项操作');
		}
	},
	addReading : function(button) {
		if (session.authority.indexOf('b351002') > -1)	{		
		    var formWin = createWin('添加相关阅读', 'sportreadingform');
		    var p_id = this.getSportreadingViewForm().form.findField('id').getValue();
	...
	edit : function(grid, rowIndex, colIndex, actionItem, event, record, row) {
		if (session.authority.indexOf('b351003') > -1) {
			var formWin = createWin('修改生涯规划消息', 'wx_sportform');
			formWin.down('form').loadRecord(record);
	...
	editReading : function(grid, rowIndex, colIndex, actionItem, event, record, row) {
		if (session.authority.indexOf('b351003') > -1) {
			var formWin = createWin('修改相关阅读', 'sportreadingform');
			formWin.down('form').loadRecord(record);
	...
	deleteItems : function(button) {
		if (session.authority.indexOf('b351004') > -1) {
			var grid = button.up('grid');
	...
	submitReading : function(button) {
		...
		var win = button.up('window');
		var grid = this.getSportreadingGrid();	
		var freshfn = function() {
			win.close();
			grid.getStore().reload();
		};
	 ...
	 reading:function(grid, rowIndex, colIndex, actionItem, event, record, row) {
	        if (session.authority.indexOf('b351001') > -1) {
	            var formWin = createWin('相关阅读管理', 'sportreadingviewform');
