Ext.define('manage.controller.Wx_benefit', {
	extend : 'Ext.app.Controller',
	refs : [ {
		ref : 'wx_benefitGrid',
		selector : 'wx_benefitgrid'
	},{
		ref : 'benefitreadingGrid',
		selector : 'benefitreadinggrid'
	},{
		ref : 'benefitreadingViewForm',
		selector : 'benefitreadingviewform'
	}],
	views : ['wx_benefit.Form','wx_benefit.ViewForm','wx_benefit.BenefitReadingViewForm','wx_benefit.BenefitReadingForm','wx_benefit.BenefitViewForm'],
	init : function(app) {
		this.control({
			'wx_benefitgrid button[action=insert]' : {
				click : this.add
			},
			'benefitreadinggrid button[action=addReading]' : {
				click : this.addReading
			},
			'wx_benefitgrid button[action=query]' : {
				click : this.query
			},
			'wx_benefitgrid button[action=delete]' : {
				click : this.deleteWx_researchs
			},
			'benefitreadinggrid button[action=deleteReading]' : {
				click : this.deleteItems
			},
			'wx_benefitgrid actioncolumn[action=edit]' : {
				click : this.edit
			},
			'benefitreadinggrid actioncolumn[action=edit]' : {
				click : this.editReading
			},
			'wx_benefitgrid actioncolumn[action=yes]' : {
				click : this.yes
			},
			'benefitreadinggrid actioncolumn[action=showOrNot]' : {
				click : this.showOrNot
			},
			'benefitreadinggrid actioncolumn[action=view]' : {
				click : this.viewReadingDetail
			},
			'wx_benefitgrid actioncolumn[action=view]' : {
				click : this.viewDetail
			},
			'wx_benefitgrid actioncolumn[action=reading]' : {
				click : this.reading
			},
			'wx_benefitform button[action=submit]' : {
				click : this.submit
			},
			'benefitreadingform button[action=submit]' : {
	...
	viewDetail : function(grid, rowIndex, colIndex, actionItem, event, record,
		...
		if (session.authority.indexOf('b351001') > -1) {
			var formWin = createWin('查看图文信息', 'wx_benefitviewform');
	...
	viewReadingDetail : function(grid, rowIndex, colIndex, actionItem, event, record,
		...
		if (session.authority.indexOf('b351101') > -1) {
			var formWin = createWin('查看图文信息', 'wx_benefitreadingviewform');
			formWin.down('form').loadRecord(record);
	...
	add : function(button) {
		if (session.authority.indexOf('b351102') > -1)
			createWin('添加图文', 'wx_benefitform');
		else {
			Ext.Msg.alert('系统提示', '您无权进行此项操作');
		}
	},
	addReading : function(button) {
		if (session.authority.indexOf('b351102') > -1)	{		
		    var formWin = createWin('添加相关阅读', 'benefitreadingform');
		    var p_id = this.getBenefitreadingViewForm().form.findField('id').getValue();
	...
	edit : function(grid, rowIndex, colIndex, actionItem, event, record, row) {
		if (session.authority.indexOf('b351103') > -1) {
			var formWin = createWin('修改生涯规划消息', 'wx_benefitform');
			formWin.down('form').loadRecord(record);
	...
	editReading : function(grid, rowIndex, colIndex, actionItem, event, record, row) {
		if (session.authority.indexOf('b351103') > -1) {
			var formWin = createWin('修改相关阅读', 'benefitreadingform');
			formWin.down('form').loadRecord(record);
	...
	deleteItems : function(button) {
		if (session.authority.indexOf('b351104') > -1) {
			var grid = button.up('grid');
	...
	deleteWx_researchs : function(button) {
    	if (session.authority.indexOf('b351104') > -1) {	
    		var grid = this.getWx_benefitGrid();
	...
	submitReading : function(button) {
		...
		var win = button.up('window');
		var grid = this.getBenefitreadingGrid();	
		var freshfn = function() {
			win.close();
			grid.getStore().reload();
		};
	 ...
	 reading:function(grid, rowIndex, colIndex, actionItem, event, record, row) {
	        if (session.authority.indexOf('b351101') > -1) {
	            var formWin = createWin('相关阅读管理', 'benefitreadingviewform');
