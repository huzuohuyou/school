Ext.define('manage.controller.ExecuteWaitActivity', {
    extend: 'Ext.app.Controller',
    refs: [{
        ref: 'executewaitactivityGrid',
        selector: 'executewaitactivitygrid'
    },{
        ref: 'executewaitactivitystepGrid',
        selector: 'executewaitactivitystepgrid'
    },{
        ref: 'activitystudentGrid',
        selector: 'activitystudentgrid'
    }],
    views: ['executewaitactivity.Form','executewaitactivity.ViewForm','executewaitactivity.WorkOrderForm',
    	'executewaitactivity.StepForm','executewaitactivity.ActivityStudentForm'],
    init: function(app) {
        this.control({
            'executewaitactivitygrid button[action=insert]': {
                click: this.add
            },
			'executewaitactivitygrid button[action=query]' : {
				click : this.query
			},
            'executewaitactivitygrid actioncolumn[action=edit]': {
                click: this.edit
            },
            'executewaitactivitygrid button[action=delete]': {
                click: this.deleteItems
            },
            'executewaitactivitygrid button[action=submit]': {
                click: this.submit
            },
            'executewaitactivitygrid actioncolumn[action=viewContract]': {
                click: this.viewContract
            },
            'executewaitactivitygrid actioncolumn[action=studentlist]': {
                click: this.studentlist
            },
            'executewaitactivitygrid actioncolumn[action=start]': {
                click: this.start
            },
            'executewaitactivitygrid actioncolumn[action=step]': {
                click: this.step
            },
            'executewaitactivitygrid actioncolumn[action=end]': {
                click: this.end
            },
            'executewaitactivitygrid actioncolumn[action=confirm]': {
                click: this.confirm
            },
            'executewaitactivitygrid actioncolumn[action=view]': {
                click: this.viewDetail
            },
            'executewaitactivitygrid actioncolumn[action=workOrder]': {
                click: this.workOrder
            },
            'executewaitactivitygrid actioncolumn[action=workOrderPass]': {
                click: this.workOrderPass
            },
            'executewaitactivitystepgrid actioncolumn[action=activityStepExecute]': {
                click: this.activityStepExecute
            },
            'executewaitactivityworkorderform button[action=submit]': {
    ...
    studentlist:function(grid, rowIndex, colIndex, actionItem, event, record, row) {
            var formWin = createWin('学生名单', 'activitystudentform');
    ...
    viewDetail : function(grid, rowIndex, colIndex, actionItem, event, record,row) {
		...
		var formWin = createWin('查看活动', 'executewaitactivityviewform');
		formWin.down('form').loadRecord(record);
		
	},
    add: function(button) {
        if (session.authority.indexOf('b300102') > -1) createWin('添加活动', 'executewaitactivityform');
        else {
    ...
    edit: function(grid, rowIndex, colIndex, actionItem, event, record, row) {
        if (session.authority.indexOf('b300103') > -1) {
            var formWin = createWin('修改活动', 'executewaitactivityform');
            formWin.down('form').loadRecord(record);
        } else {
            Ext.Msg.alert('系统提示', '您无权进行此项操作');
        }
    ...
    workOrder: function(grid, rowIndex, colIndex, actionItem, event, record,
		...
		var formWin = createWin('活动工单', 'executewaitactivityworkorderform');
    ...
    step: function(grid, rowIndex, colIndex, actionItem, event, record, row) {
    	var formWin = createWin('活动步骤', 'executewaitactivitystepform');
    	var grid = formWin.down('grid');
    ...
    submit: function(button) {
        ...
        var grid = this.getExecutewaitactivityGrid();
